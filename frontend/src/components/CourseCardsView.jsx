      ...
      <CardContent>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
          ...
          <Chip 
            ...
            label={`${course?.SoLuongSinhVienDangKy} Students`} 
            size="small"
            sx={{ bgcolor: '#f5f5f5' }}
          />
          <Chip 
            icon={<PersonIcon fontSize="small" />} 
            label={course?.TenGiangVien} 
            size="small"
            sx={{ bgcolor: '#f5f5f5' }}
          />
        </Box>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Typography variant="body2" sx={{ fontSize: '0.8rem' }}>
            {course.reviewsCompleted} of {course?.DanhGiaList?.length} reviews
          </Typography>
        </Box>
        
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center'}}>
            <Rating value={course.TrungBinhSoSao || 0} precision={0.5} readOnly size="small" />
...
const CourseCardsView = ({ courses, searchTerm, year, onSelectCourse }) => {
      ...
      {filteredCourses.length > 0 ? (
        <Grid container spacing={3}>
          {filteredCourses.map((course) => (
            <Grid item xs={12} sm={6} md={4} key={course?.MaKhoaHoc}>
              <CourseCard 
                course={course} 
