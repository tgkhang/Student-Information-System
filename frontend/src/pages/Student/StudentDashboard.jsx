...
const subjectScoresData = [
  {
    ...
    name: "Introduction to Programming",
    assessments: [
      { name: "Quiz 1", score: 85, maxScore: 100, weight: 10, date: "2023-09-15" },
      { name: "Assignment 1", score: 92, maxScore: 100, weight: 15, date: "2023-09-25" },
      { name: "Midterm Exam", score: 88, maxScore: 100, weight: 30, date: "2023-10-20" },
      { name: "Project", score: 95, maxScore: 100, weight: 20, date: "2023-11-15" },
      { name: "Final Exam", score: 90, maxScore: 100, weight: 25, date: "2023-12-10" },
    ],
    finalGrade: "A",
    status: "passed",
  },
  {
    ...
    name: "Calculus I",
    assessments: [
      { name: "Quiz 1", score: 75, maxScore: 100, weight: 10, date: "2023-09-18" },
      { name: "Quiz 2", score: 80, maxScore: 100, weight: 10, date: "2023-10-05" },
      { name: "Midterm Exam", score: 78, maxScore: 100, weight: 30, date: "2023-10-22" },
      { name: "Assignment", score: 85, maxScore: 100, weight: 20, date: "2023-11-10" },
      { name: "Final Exam", score: 82, maxScore: 100, weight: 30, date: "2023-12-12" },
    ],
    finalGrade: "B+",
    status: "passed",
  },
  {
    ...
    name: "Data Structures",
    assessments: [
      { name: "Quiz 1", score: 65, maxScore: 100, weight: 10, date: "2024-02-10" },
      { name: "Assignment 1", score: 70, maxScore: 100, weight: 15, date: "2024-02-25" },
      { name: "Midterm Exam", score: 55, maxScore: 100, weight: 30, date: "2024-03-15" },
      { name: "Project", score: 60, maxScore: 100, weight: 20, date: "2024-04-05" },
      { name: "Final Exam", score: 58, maxScore: 100, weight: 25, date: "2024-05-10" },
    ],
    finalGrade: "D",
    status: "failed",
  },
...
export default function Dashboard() {
  ...
  const [selectedSubject, setSelectedSubject] = React.useState(subjectScoresData[0]?.subjectId || "")
  ...
  const selectedSubjectData = subjectScoresData.find((subject) => subject.subjectId === selectedSubject)
  ...
  const calculateTotalScore = (subject) => {
    return subject.assessments.reduce((total, assessment) => total + calculateWeightedScore(assessment), 0)
  }
                  ...
                  <Card elevation={0} sx={{ borderRadius: 2, height: "100%" }}>
                    <CardContent>
                      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mb: 3 }}>
                        ...
                        <Chip label={studentProfile.status} color="success" sx={{ mt: 1, fontWeight: "medium" }} />
                      </Box>
                ...
                <>
                  <Card elevation={0} sx={{ borderRadius: 2, mb: 3, p: 2 }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                      <Box>
                        <Typography variant="h6" sx={{ fontWeight: "bold" }}>
                          {selectedSubjectData.subjectId} - {selectedSubjectData.name}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Final Grade:{" "}
                          <Typography
                            ...
                            sx={{
                              fontWeight: "bold",
                              color: getGradeColor(selectedSubjectData.finalGrade),
                            }}
                          >
                      ...
                      <Chip
                        label={selectedSubjectData.status === "passed" ? "Passed" : "Failed"}
                    ...
                    <Box sx={{ mt: 3 }}>
                      <Typography variant="subtitle1" sx={{ fontWeight: "bold", mb: 2 }}>
                        Overall Score: {calculateTotalScore(selectedSubjectData).toFixed(2)}%
                      </Typography>
                      <Box
                        sx={{
                          width: "100%",
                      ...
                      >
                        <Box
                          sx={{
                            width: `${calculateTotalScore(selectedSubjectData)}%`,
                            height: "100%",
                            backgroundColor:
                              calculateTotalScore(selectedSubjectData) >= 80
                                ? "success.main"
                                : calculateTotalScore(selectedSubjectData) >= 60
                                  ? "warning.main"
                                  : "error.main",
                            borderRadius: 5,
                          }}
                  ...
                  <TableContainer component={Paper} elevation={0} sx={{ borderRadius: 2 }}>
                    <Table sx={{ minWidth: 650 }}>
                      ...
                      <TableBody>
                        {selectedSubjectData.assessments.map((assessment, index) => (
                          <StyledTableRow key={index}>
                            <TableCell sx={{ fontWeight: "medium" }}>{assessment.name}</TableCell>
                            <TableCell>{formatDate(assessment.date)}</TableCell>
                            <TableCell align="center">{assessment.score}</TableCell>
                            <TableCell align="center">{assessment.maxScore}</TableCell>
                            <TableCell align="center">
                              <Typography
                                sx={{
                                  fontWeight: "medium",
                                  color:
                                    (assessment.score / assessment.maxScore) * 100 >= 80
                                      ? "success.main"
                                      : (assessment.score / assessment.maxScore) * 100 >= 60
                                        ? "warning.main"
                                        : "error.main",
                                }}
                              >
                            ...
                            <TableCell align="center">
                              <Typography sx={{ fontWeight: "bold" }}>
                                {calculateWeightedScore(assessment).toFixed(2)}
                              </Typography>
                            </TableCell>
                          </StyledTableRow>
                        ))}
