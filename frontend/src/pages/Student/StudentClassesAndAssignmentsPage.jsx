...
export default function StudentClassesAndAssignmentsPage() {
  ...
  const handleChange = (event, newValue) => {
    setValue(newValue)
    navigate(`/student/classAndAssignment${newValue > 0 ? `?tab=${newValue}` : ""}`)
  }
  ...
  const filteredAssignments = assignmentsData.filter((assignment) => {
    ...
    if (assignmentFilter === "pending") return assignment.status === "pending" && matchesSearch
    if (assignmentFilter === "submitted") return assignment.status === "submitted" && matchesSearch
    if (assignmentFilter === "graded") return assignment.status === "graded" && matchesSearch
  ...
  const pendingAssignmentsCount = assignmentsData.filter((a) => a.status === "pending").length
          ...
          <Grid item xs={12} md={4}>
            <Card elevation={0} sx={{ borderRadius: 2 }}>
              <CardContent>
                ...
                <Typography variant="h3" sx={{ fontWeight: "bold", mb: 1 }}>
                  {registeredClassesData.filter((c) => c.status === "active").length}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Total Credits:{" "}
                    ...
                    {filteredAssignments.map((row) => (
                      <StyledTableRow key={row.id}>
                                  ...
                                  sx={{
                                    color: isOverdue(row.dueDate)
                                      ...
                                      : isDueSoon(row.dueDate)
                                        ? "warning.main"
                                        : "text.secondary",
                                    fontWeight: isOverdue(row.dueDate) || isDueSoon(row.dueDate) ? "bold" : "normal",
                                  }}
                                >
                        ...
                        <TableCell>
                          <Chip
                            label={
                              row.status === "pending" ? "Pending" : row.status === "submitted" ? "Submitted" : "Graded"
                            }
                            color={getStatusColor(row.status)}
                            size="small"
                            sx={{ fontWeight: "medium" }}
                          ...
                          {row.grade !== null ? (
                            <Typography
                              ...
                              sx={{
                                fontWeight: "bold",
                                color:
                                  row.grade >= 80 ? "success.main" : row.grade >= 60 ? "warning.main" : "error.main",
                              }}
                            >
                        ...
                        <TableCell>
                          <Chip
                            icon={row.priority === "high" ? <PriorityHighIcon /> : null}
