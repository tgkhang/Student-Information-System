...
export default function StudentClassRegistrationPage() {
  ...
  const [registeredClasses, setRegisteredClasses] = React.useState(registeredClassesData)
  ...
  const handleDeleteConfirm = () => {
    if (classToDelete) {
      setRegisteredClasses(registeredClasses.filter((item) => item.id !== classToDelete.id))
      setSuccessAlert(`Successfully removed ${classToDelete.subjectName} from your schedule.`)
  ...
  const handleEnrollConfirm = () => {
    if (classToEnroll) {
      ...
      if (!registeredClasses.some((c) => c.id === classToEnroll.id)) {
        ...
        setSuccessAlert(`Successfully enrolled in ${classToEnroll.subjectName}.`)
        setTimeout(() => setSuccessAlert(""), 3000)
      }
    }
  ...
  const totalRegisteredCredits = registeredClasses.reduce((sum, item) => sum + item.credit, 0)
  const maxCredits = 18
    ...
    <Box sx={{ display: "flex" }}>
      ...
      >
            ...
            <Card elevation={0} sx={{ borderRadius: 2 }}>
              <CardContent>
                ...
                >
                  <Box
                    sx={{
                      width: `${(totalRegisteredCredits / maxCredits) * 100}%`,
                      height: "100%",
                  ...
                  <TableBody>
                    {registeredClasses.map((row) => (
                      <StyledTableRow key={row.id}>
                        ...
                        <TableCell sx={{ fontWeight: "medium" }}>{row.subjectName}</TableCell>
                        ...
                        <TableCell>
                          <Chip
                            label={row.status === "confirmed" ? "Confirmed" : "Pending"}
                ...
                <TableBody>
                  {filteredClasses.map((row) => (
                    <StyledTableRow key={row.id}>
                      ...
                      <TableCell align="center">
                        <Tooltip title={row.availability === "full" ? "Class is full" : "Enroll in this class"}>
                          <span>
                            <Button
                              variant="contained"
                              color="primary"
                              ...
                              disabled={row.availability === "full" || registeredClasses.some((c) => c.id === row.id)}
                            >
                              Enroll
                            </Button>
                          </span>
        ...
        <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
          ...
          <DialogContent>
            <DialogContentText>
              Are you sure you want to remove <strong>{classToDelete?.subjectName}</strong> from your schedule?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
        ...
        <Dialog open={enrollDialogOpen} onClose={() => setEnrollDialogOpen(false)}>
          ...
          <DialogContent>
            <DialogContentText>
              Are you sure you want to enroll in <strong>{classToEnroll?.subjectName}</strong>?
            </DialogContentText>
            {classToEnroll && (
