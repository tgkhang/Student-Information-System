...
export class SinhVienService {
    ...
    async addThongBaoToStudentsInKhoaHoc(studentIds: Types.ObjectId[], thongBaoId: Types.ObjectId): Promise<void> {
        await this.sinhVienModel.updateMany(
        { _id: { $in: studentIds } },
        { $push: { ThongBao: { thongBaoId, isRead: false } } },
        );
    ...
    async getStudentNoti(MSSV: string){
        ...
        console.log(MSSV);
        const sinhVien = await this.sinhVienModel.findOne({mssv: MSSV}).populate('ThongBao.thongBaoId').exec();
        if (!sinhVien) {
          throw new NotFoundException('Không tìm thấy sinh viên.');
        }
